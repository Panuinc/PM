...
/**
 * สร้าง Flex Message สำหรับ Visitor Check-In
 */
export function buildVisitorCheckInFlex(visitor, contactUser) {
  const reasonLabels = {
    Shipping: "📦 Shipping",
    BillingChequeCollection: "💰 Billing / Cheque Collection",
...
export async function notifyVisitorCheckIn(visitor, contactUser) {
  ...
  try {
    ...
    if (isLocalhost()) {
      console.warn(
        "Running on localhost - images and action buttons will be disabled in LINE Flex Message"
      );
    }
