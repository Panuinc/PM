...
export async function GetAllPermissionGroupUseCase(page = 1, limit = 1000000) {
  ...
  const permissionGroups = await PermissionGroupService.getAllPaginated(
    skip,
    limit
  );
  ...
  logger.info({ message: "GetAllPermissionGroupUseCase success", total });
...
export async function GetPermissionGroupByIdUseCase(permissionGroupId) {
  ...
  const permissionGroup = await PermissionGroupService.getById(
    permissionGroupId
  );

  if (!permissionGroup)
    throw { status: 404, message: "PermissionGroup not found" };
...
export async function CreatePermissionGroupUseCase(data) {
  ...
  const duplicate =
    await PermissionGroupValidator.isDuplicatePermissionGroupName(
  ...
  const permissionGroup = await PermissionGroupService.create({
    ...
    permissionGroupCreatedBy: parsed.data.permissionGroupCreatedBy,
    permissionGroupCreatedAt: getLocalNow(),
  });
...
export async function UpdatePermissionGroupUseCase(data) {
  ...
  const existing = await PermissionGroupService.getById(
    parsed.data.permissionGroupId
  );
  ...
  const updatedPermissionGroup = await PermissionGroupService.update(
    parsed.data.permissionGroupId,
    {
      permissionGroupName: parsed.data.permissionGroupName.trim(),
      permissionGroupOrder: parsed.data.permissionGroupOrder,
      permissionGroupUpdatedBy: parsed.data.permissionGroupUpdatedBy,
      permissionGroupUpdatedAt: getLocalNow(),
    }
  );
